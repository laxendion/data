...
#    my ( $zquery, $squery ) = _build_query( $pars );
    ...
    my $db_data = $servers[0]->{db};

    my ( $zquery, $squery ) = _build_query( $pars , $db_data);
...
sub _build_query {
    ...
    #one of the unit tests is based on that assumption
	    ...
	    if($db_data eq 'tl_marc'){
              my $u = utf8($val); 
	      my $tmp = encode("gb18030",decode("utf8",$u->utf8));
	      $qry_build->{$k} =~ s/#term/$tmp/g; 
              $zquery .= $qry_build->{$k};
              $squery .= "[$k]=\"$tmp\" and ";
	    }
            else 
...
sub _handle_one_result {
    ...
    if( $servhref->{db} eq 'tl_marc') {
       $raw= trans($zoomrec->raw());
    }
    else 
    {
       $raw= $zoomrec->raw();
...
sub trans {
  ...
  my $first_pos = substr($row, 12, 5);
  my $directories = substr($row, 24, ($first_pos - 24 - 1));
  ...
  for(my $i = 0; $i < (length($directories)/12); $i++) {
    
     my $directory = substr($row, $pos, 12);
     $pos = $pos + 12;

     my $tag = substr($directory, 0, 3);
     my $tag_len = substr($directory, 3, 4);
    
     my $tag_data = encode("utf8",decode("gb18030",substr($row, $base, $tag_len))); 
     my $new_taglen = sprintf("%04d",length($tag_data));

     my $newpos = sprintf("%05d", $new_pos);
     $new_directory = $new_directory . $tag . $new_taglen . $newpos; 
     $new_pos = $new_pos + $new_taglen;
     $new_marc = $new_marc . $tag_data;

     $base = $base + $tag_len;
  }


   my $newmarcdata = sprintf("%05d", (24 + length($new_directory . '\x1E') + length($new_marc)));
   $newmarcdata = $newmarcdata . substr($row, 5, 7);
   $newmarcdata = $newmarcdata . sprintf("%05d", (24 + length($new_directory . '\x1E' )));
   $newmarcdata = $newmarcdata . substr($row, 17, 7);
   $newmarcdata = $newmarcdata . $new_directory . '\x1E' . $new_marc . '\x1D';
