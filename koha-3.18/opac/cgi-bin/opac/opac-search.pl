...
foreach (@newresults) {  
    ...
    my $isbn = GetNormalizedISBN(undef,$record,C4::Context->preference("marcflavour"));
    #warn "ISBN = " . $isbn;
    $isbn =~ s/-//g;
    #warn "new isbn = " . $isbn;
    my $douban = new Getdouban();
    #warn "image = " . $douban->get_cover($isbn);
    $_->{douban_url} =  $douban->get_cover($isbn);
