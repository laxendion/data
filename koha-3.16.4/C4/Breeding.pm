...
sub Z3950Search {
    ...
    foreach my $servid (@id) {
        ...
        while (my $server = $sth->fetchrow_hashref) {
            ...
            #warn "===============================" . 
            #"DB = " .  $server->{db} . ", user = " . $server->{userid} . 
            #", passwd = " . $server->{password} . ", syntax = " . $server->{syntax} .
            #", timeout = " . $server->{timeout} ."\n";
            #warn "ZOOM::Connection ================= $oConnection[$s]";
            #warn "options1 ================= " . Dumper($option1);
            $oConnection[$s]= create ZOOM::Connection($option1);
            #warn "option1 ------------- " . Dumper($oConnection[$s]);
            #warn "option1 ============ $option1";
            $oConnection[$s]->connect( $server->{host}, $server->{port} );
            $serverinfo[$s]->{host}= $server->{host};
            #warn " Server ===========" . $serverinfo[$s]->{host};
            $serverinfo[$s]->{name}= $server->{name};
            $serverinfo[$s]->{encd}= $server->{encoding} // "iso-5426";
            #warn "encod ----- " .  $serverinfo[$s]->{encd};

            #warn " Server =============== " . Dumper($serverinfo[$s]);
            $s++;
        }    ## while fetch
    }    # foreach
    my $nremaining  = $s;
    ...
    for ( my $z = 0 ; $z < $s ; $z++ ) {
        ...
        #arn "o Result =============== " . Dumper($oResult[$z]);
        #warn "o Result =============== " .  $oResult[$z];
    }
